"""
Scraper for Agenzia Entrate Circolari archive
=============================================

This script recursively navigates through the **Archivio Circolari** section
...
attachments for a single circular) are followed automatically, ensuring that
...
``CircolariScraper`` in your own code.  See the ``main`` function at the
...
dynamically by following links that include the ``circolari`` keyword in
...
class CircolariScraper:
    """Scraper for the Agenzia Entrate circolari archive.

    The scraper starts from the root archive page, discovers year and month
    ...
    def should_follow_link(self, url: str, current_url: str) -> bool:
        ...
        circolari archive are considered.  The current implementation allows
        navigation if the URL contains the substring ``circolari`` in its
        ...
        if not url.startswith(self.BASE_URL):
            return False
        url_lower = url.lower()
        # Only follow pages that include 'circolari' keyword
        if "circolari" not in url_lower:
            return False
        # Exclude sections unrelated to the circolari archive
        exclude_patterns = [
            "risoluzioni",
            "interpello",
            "provvedimenti",
...
async def main() -> None:
    ...
    scraper = CircolariScraper(bucket_name=bucket_name, max_concurrent=10, base_folder="circolari")
