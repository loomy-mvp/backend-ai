"""
DPR Scraper for Normattiva
==========================

This script navigates the Normattiva advanced search to find DPR 
(Decreto del Presidente della Repubblica) documents, uses AWS Bedrock 
to determine their relevance for accountants (commercialisti), and 
saves relevant documents to Google Cloud Storage.

The scraper:
1. Uses Selenium to navigate Normattiva's JavaScript-heavy interface
2. Filters for DPR document types
...
class DPRScraper:
    """Scraper for DPR documents from Normattiva.
    
    This scraper navigates the Normattiva advanced search interface using
    Selenium, filters for DPR documents, evaluates their relevance using
    ...
    def _list_existing_urls(self) -> set:
        """List all existing DPR URLs from GCS metadata files."""
        existing_urls = set()
        try:
    ...
    def _call_bedrock(self, subtitle: str) -> dict:
        ...
        """
        prompt = RELEVANCE_PROMPT.format(dpr_subtitle=subtitle)
    ...
    def _generate_filename(self, title: str) -> Optional[str]:
        """Generate filename from DPR title.
        
        Expected title format: "DECRETO DEL PRESIDENTE DELLA REPUBBLICA 4 dicembre 2025, n. 205"
        Output format: "dpr_04_dicembre_2025_n_205.txt"
        """
    ...
    def _check_not_published(self, html: str) -> bool:
        """Check if DPR is not yet published."""
        return "NON ANCORA ESISTENTE O VIGENTE" in html
    
    def _scrape_dpr_text(self, dpr_url: str) -> Tuple[Optional[str], List[Tuple[str, str]]]:
        """Scrape full DPR text from Normattiva URL.
        ...
        try:
            logger.info(f"  Loading DPR page: {dpr_url}")
            resp = self._fetch(session, dpr_url)
        except Exception as exc:
            logger.error(f"  Failed to load DPR page {dpr_url}: {exc}")
            return None, articles
        
        # Check if not published
        if self._check_not_published(resp.text):
            logger.info("  DPR not yet published, skipping")
            return None, []
    ...
    def _navigate_and_search(self) -> List[Dict]:
        """Navigate Normattiva and execute DPR search.
        ...
        try:
            ...
            # Select "DECRETO DEL PRESIDENTE DELLA REPUBBLICA"
            logger.info("Selecting 'DECRETO DEL PRESIDENTE DELLA REPUBBLICA'...")
            ...
            # Now try to find and click the DPR option
            # IMPORTANT: Must match EXACTLY "DECRETO DEL PRESIDENTE DELLA REPUBBLICA" 
            # and NOT "DECRETO DEL PRESIDENTE DEL CONSIGLIO DEI MINISTRI" (DPCM)
            js_click_script = """
            var searchText = 'DECRETO DEL PRESIDENTE DELLA REPUBBLICA';
            ...
            for (var s = 0; s < selectors.length; s++) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    // Must match exactly "DECRETO DEL PRESIDENTE DELLA REPUBBLICA"
                    // Exclude DPCM (CONSIGLIO DEI MINISTRI)
                    if (text.includes(searchText) && !text.includes('CONSIGLIO')) {
            ...
            if js_result == 'not found':
                ...
                raise Exception(
                    "CRITICAL: Could not find 'DECRETO DEL PRESIDENTE DELLA REPUBBLICA' option in dropdown. "
    ...
    def scrape(self) -> Dict:
        ...
        if not entries:
            logger.warning("No DPR entries found")
            return self.stats
        ...
        for i, entry in enumerate(entries, 1):
            ...
            title, articles = self._scrape_dpr_text(entry["url"])
            
            if articles is None or len(articles) == 0:
                # Check if it was because DPR is not published
                if title is None:
