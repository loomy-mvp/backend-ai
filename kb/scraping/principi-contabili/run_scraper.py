"""Runner script for the Principi Contabili scraper."""
...
from src.scrape_principi_contabili import PrincipiContabiliScraper

logger = logging.getLogger(__name__)


async def main(bucket_name: str, base_folder: str, max_concurrent: int, log_level: str) -> None:
    logger.info("🚀 Starting Principi Contabili scraper")
    ...
    try:
        scraper = PrincipiContabiliScraper(
        ...
        logger.info("\n" + "=" * 80)
        logger.info("✅ PRINCIPI CONTABILI SCRAPING COMPLETED")
        logger.info("=" * 80)
        logger.info("🔗 Links discovered: %d", results.get("links_discovered", 0))
        logger.info("📄 Files processed: %d", results.get("files_processed", 0))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the Principi Contabili scraper and upload files to GCS"
    )
    parser.add_argument(
        "--bucket-name",
    ...
    parser.add_argument(
        "--base-folder",
        default="principi-contabili",
        help="Base folder path in GCS bucket (default: principi-contabili)",
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
        default=3,
        help="Number of concurrent HTTP requests (default: 3)",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
