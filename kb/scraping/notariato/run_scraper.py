...
Runs the Notariato scraper and exits
"""
...
async def main(bucket_name: str, base_folder: str, max_concurrent: int, log_level: str):
    ...
    logger.info(f"🚀 Starting Notariato scraper")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the Notariato scraper and upload files to GCS"
    )
    parser.add_argument(
        "--bucket-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="notariato",
        help="Base folder path in GCS bucket (default: notariato)"
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
