...
Runs the DLGS (Decreto Legislativo) scraper and exits
"""
...
def main(bucket_name: str, base_folder: str, log_level: str, limit: int = 0, start_page: int = 1, use_stop_condition: bool = True):
    ...
    logger.info("🚀 Starting DLGS (Decreto Legislativo) scraper")
    ...
    try:
        ...
        logger.info("="*80)
        logger.info(f"📄 Total DLGS found: {results['total_found']}")
        logger.info(f"🔍 DLGS checked for relevance: {results['total_checked']}")
        logger.info(f"✨ Relevant DLGS: {results['relevant_count']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the DLGS scraper and upload files to GCS"
    )
    parser.add_argument(
        "--bucket-name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="dlgs",
        help="Base folder path in GCS bucket (default: dlgs)"
    )
    parser.add_argument(
        "--log-level",
        type=str,
